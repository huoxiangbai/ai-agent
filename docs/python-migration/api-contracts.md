...
### Admin featured write contract — measured 2026-09-25 (phase 4)

Evidence: `tests/contract/cases/phase4-featured-admin.json` (15 cases) against `tests/contract/golden/java-featured-admin.json` — `--record-java` re-recorded byte-identically, `--golden` against live Python returned `cases=15 skipped=[] differing=[] matched=15`.

| Fact | Measured behaviour |
|---|---|
| Rejection shape | Rejections are **HTTP 200 + `code`**, except Java's uncaught `IllegalArgumentException`, which escapes as the four-key `500` body above (`sessionId 不能为空`, missing/unknown `featuredId`). Three such probes are pinned in `tests/cutover/admin_probes.py::EXPECT_STATUS`; a 5xx outside that map still fails the drill. |
| `query-list` pagination | **Absent key ≠ JSON null.** Absent `pageNo`/`pageSize` → `1`/`10` (Lombok `@Builder.Default` installed by the no-arg constructor Jackson binds through; verified by `javap`). A *present* JSON null → `0` via the setter, then `Math.max(1, …)` yields offset 0 / limit 1. So `{}` pages at width 10 and `{"pageSize":null}` at width 1 — 8 probe bodies, `differences=0`. |
| `update` with a valid `featuredId` but **no `sessionId`** | `data:false`, HTTP 200, row unchanged, and **no SQL at all** — `FeaturedConversationRepository.upsert`'s entry guard (`command==null \|\| blank(featuredId) \|\| blank(sessionId)`) returns `false` before issuing a statement. Caught only by response comparison; the cloned-DB table diff was empty. |
| Null/default and unique-key semantics | `coerce_body_primitive_*` reproduces Jackson: JSON null on a primitive → its Java zero, type mismatch → the same 500 four-key rejection. `upsert`'s `ON DUPLICATE KEY UPDATE` column list and the conditional-update rowcount (`0001`/`data:false` on a zero-row update) are asserted in `tests/integration/test_featured_admin_write_repository.py`. |
| Cache/registry effects | none exist on this table — the acceptance scope is the table rows plus the two writer-fence layers (see `database-ownership.md`), both exercised live on 2026-09-25. |
...
Phase-2 exit requires "every route covered or explicitly deferred". Today **29 cases** in four files cover the initial set (7) and the featured surface (3 public GETs + 4 transport-error shapes + 15 admin, `phase4-featured-admin.json`); the rest are recorded here so coverage is never overstated. `Owner phase` is when the deferred item becomes blocking.
...
| ~~Admin featured CRUD~~ | ~~`POST\|PUT\|POST` × 5 on `/api/v1/admin/featured-conversations`~~ | ~~write domain + cache/registry effects~~ | **Covered 2026-09-25** — `phase4-featured-admin.json`, 15 cases, `skipped: []`; see "Admin featured write contract" above |
