...
- Deployment: Nginx (`docker/nginx.conf`) fronts the React SPA and proxies `/web/`, `/api/`, `/data/` to the Java backend on port 8100, plus `/tool/` to `reactor-tool` on port 1601. `reactor-sandbox` runs on 1602. The Python coexistence backend `reactor-backend-python` listens on `${PYTHON_BACKEND_PORT:-8200}`.
  **Since Phase 3B there are two `/api/` upstreams.** `upstream reactor_backend` (Java, `:8100`) remains the default via `location /api/`. `upstream reactor_backend_python` (Python, `:8200`) is reached only through the fragment `docker/nginx-featured-python.d/featured-conversations.conf`, included by a glob from `nginx.conf`. The fragment holds exactly three locations — two exact matches (`/api/agent/featured-conversations/home`, `/api/agent/featured-conversations`) and one single-segment regex (`~ ^/api/agent/featured-conversations/[^/]+$`) — and is the **entire** cutover switch: move the file aside + `nginx -t` + reload to return to Java. The directory is bind-mounted into the frontend container (`docker-compose.yml`) so a rollback survives a container restart. Trailing-slash and multi-segment near-misses deliberately stay on Java so their 404 keeps Spring's error body.
- Python side today: `GET /internal/health/live`, `GET /internal/health/ready`, (non-prod only) `GET /docs`, **three public business routes** — `GET /api/agent/featured-conversations/home`, `GET /api/agent/featured-conversations`, `GET /api/agent/featured-conversations/{featuredId}` (Phase 3A implemented, 3B cut over) — and, since phase 4 (2026-09-25), the **five admin routes** on `/api/v1/admin/featured-conversations` (`create`/`update`/`online/{featuredId}`/`offline/{featuredId}`/`query-list`). Zero SSE server endpoints.
...
| `/api/agent/featured-conversations` | `GET /home`; `GET /`; `GET /{featuredId}` | read-only featured content (P3 pilot) | 3 | featured-read | HTTP contract cases `featured-home-default`, `featured-list-first-page`, `featured-detail-fixture` (`tests/contract/cases/phase3-featured.json`, golden `java-featured.json`); Python `tests/unit/test_featured_routes.py` + `tests/integration/test_featured_read_repository.py`; Java `FeaturedConversationPublicQueryApplicationServiceTest` 13/13 | cutover owner |
...
| `/api/v1/admin/featured-conversations` | `POST /create`; `PUT /update`; `POST /online/{featuredId}`; `POST /offline/{featuredId}`; `POST /query-list` | admin CRUD | reads `3` (only if proven side-effect-free); writes `4` | featured-admin | **done (2026-09-25)** — cloned-DB parity `differences=0` over 32 cases, contract 15/15 `skipped=[]`, drill cutover 0.087s / rollback 0.512s | **Python owns every operation** — exact-path Nginx fragment `docker/nginx-featured-python.d/featured-admin.conf`; two-layer writer fence (`reactor_py_featured_writer` + `REACTOR_PY_FEATURED_ADMIN_WRITE_OWNER`, fail-closed) |
...
- Error shaping: there is **no** `@ControllerAdvice`, `@ExceptionHandler` or `ErrorController`. Each handler catches and maps into the `{code, info, data}` envelope, so business errors normally stay HTTP 200. Anything that escapes a handler — a `MethodArgumentTypeMismatchException`, an unmapped path, an unsupported method, an unhandled exception — ends in `response.sendError(...)` and comes back from Spring Boot's auto-configured `BasicErrorController` as the four-key body measured in `api-contracts.md`. A Python reimplementation must reproduce **both** shapes on the right occasions, and must not introduce a global HTTP error policy or FastAPI 307/422 drift.
- Python coexistence backend (`backend-python/`): env-only config with prefix `REACTOR_PY_` — `APP_NAME`, `ENVIRONMENT`, `HOST`, `PORT` (default 8200), `LOG_LEVEL`, `DATABASE_URL`, `MYSQL_HOST`, `MYSQL_PORT`, `MYSQL_DATABASE`, `MYSQL_USER`, `MYSQL_PASSWORD`, `MYSQL_POOL_SIZE`, `MYSQL_MAX_OVERFLOW`, `MYSQL_POOL_RECYCLE_SECONDS`, `DATABASE_READY_TIMEOUT_SECONDS`. Secrets are `SecretStr` with `repr=False`; the DSN is built with `quote_plus`. Compose overrides the port via `PYTHON_BACKEND_PORT`.
  - **Since 2026-09-23 compose sets `REACTOR_PY_MYSQL_USER: ${REACTOR_PY_MYSQL_USER:-reactor_py_ro}` and `REACTOR_PY_MYSQL_PASSWORD: ${REACTOR_PY_MYSQL_PASSWORD:?...}`** — deliberately *not* the Java `MYSQL_USER`/`MYSQL_PASSWORD`. The Python service must only ever hold the read-only account; pointing these at `reactor` silently removes the single-writer technical enforcement (see `database-ownership.md` and R-22).
- Python modules added by the phase-3A featured-read slice (all under `backend-python/src/reactor_backend/`): `api/{coercion,presenters}.py`, `api/routers/featured_conversations.py`, `application/featured_conversation_query.py`, `domain/{context_usage,execution_ledger_constants,featured_conversation,gen_ui_schema,history_replay,ledger_types,replay_projector,summary_resolver,time_format,tool_projectors}.py`, `infrastructure/database/connection.py`, `infrastructure/llm_model_window.py`, `infrastructure/repositories/`. Tests: `tests/unit/test_tool_projectors.py` (50 key-order cases) among others; `tests/integration/{test_featured_read_repository,test_readonly_account_denies_writes}.py`; `tests/contract/cases/phase3-featured.json` + `tests/contract/golden/java-featured.json`. **`gen_ui_schema.py` and the rewritten `tool_projectors.py` are source-faithful but not golden-covered** — see the tool-frame table in `api-contracts.md`.
- Phase 3B additions: `presenters.spring_error_response` (Spring `BasicErrorController` body), a `StarletteHTTPException` handler and the 500 handler both rewritten to that shape, `middleware.JAVA_COMPAT_VARY` (the `CorsFilter` `Vary` shim) and echo-only `X-Request-ID`. Contract evidence: `tests/contract/cases/phase3-featured-errors.json` + `golden/java-featured-errors.json`. Cutover harness (test-only, never imported by `src/`): `tests/cutover/{gen_config.py,probes.py,drill.sh,README.md}`. Launch contract: `backend-python/Dockerfile` passes `--no-server-header` (R-35).
- Phase 4 additions (featured-admin write slice, 2026-09-25, all under `backend-python/src/reactor_backend/`): `api/body_coercion.py` (Jackson-shaped `null`/type coercion), `api/routers/featured_admin.py`, `application/featured_conversation_admin.py`, `domain/featured_admin.py`, `infrastructure/repositories/featured_conversation_repository.py`. Owner fence: `REACTOR_PY_FEATURED_ADMIN_WRITE_OWNER` (default `java`, fail-closed) read before any SQL. Migration: `db/migrations/20260923_provision_phase4_featured_writer_account.sql`. Nginx: `docker/nginx-featured-python.d/featured-admin.conf`. Tests: `tests/unit/{test_body_coercion,test_featured_admin,test_featured_admin_routes}.py`, `tests/integration/{test_featured_admin_write_repository,test_featured_writer_account_scope}.py`, `tests/parity/`, `tests/contract/cases/phase4-featured-admin.json` + `golden/java-featured-admin.json`, `tests/cutover/{admin_drill.sh,admin_probes.py}`.
