...
- Python side today: `GET /internal/health/live`, `GET /internal/health/ready`, (non-prod only) `GET /docs`, and **three public business routes** — `GET /api/agent/featured-conversations/home`, `GET /api/agent/featured-conversations`, `GET /api/agent/featured-conversations/{featuredId}` (Phase 3A implemented, 3B cut over). Zero SSE server endpoints.
